...
class ShotCompensationTest {
    ...
    void stationaryRobotReturnsZeroCompensation() {
        ...
        CompensationResult result = ShotCompensation.calculate(velocity, 0.0, 4.0);
    ...
    void pureLateralVelocityProducesAimLeadOnly() {
        // Target at bearing 0 rad (straight ahead in field frame)
        // Moving laterally (vy = 2 m/s to the left in field frame)
        double distance = 4.0;
        ChassisSpeeds velocity = new ChassisSpeeds(0.0, 2.0, 0.0);
        CompensationResult result = ShotCompensation.calculate(velocity, 0.0, distance);
    ...
    void pureRadialVelocityTowardTargetReducesDistance() {
        // Target at bearing 0 rad, moving forward (vx = 2 m/s toward target)
        double distance = 4.0;
    ...
    void pureRadialVelocityAwayFromTargetIncreasesDistance() {
        // Moving backward (vx = -2 m/s, away from target at bearing 0)
        double distance = 4.0;
        ChassisSpeeds velocity = new ChassisSpeeds(-2.0, 0.0, 0.0);
        CompensationResult result = ShotCompensation.calculate(velocity, 0.0, distance);
    ...
    @Test
    void differentBearingAffectsDecomposition() {
        // Target at bearing 0 vs bearing 30 degrees, robot moving forward
        double distance = 4.0;
        ChassisSpeeds velocity = new ChassisSpeeds(2.0, 0.0, 0.0);

        // bearingRad = 0 (target straight ahead)
        CompensationResult resultCenter = ShotCompensation.calculate(velocity, 0.0, distance);
        // bearingRad = -30 degrees (target offset to the right, equivalent to old heading=0 tx=30)
        CompensationResult resultOffset = ShotCompensation.calculate(velocity, Math.toRadians(-30.0), distance);
        ...
        assertNotEquals(resultCenter.aimLeadDegrees, resultOffset.aimLeadDegrees, 0.1,
                "Different bearing should change velocity decomposition");
    }
}
