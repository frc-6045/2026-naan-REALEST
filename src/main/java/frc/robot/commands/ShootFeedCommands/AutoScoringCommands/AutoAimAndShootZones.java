 ...
 */
public class AutoAimAndShootZones extends Command {
    ...
    public void execute() {
        ...
        int newPipeline = getPipeline();
        if (newPipeline!=pipeline) {
            pipeline=newPipeline;
            LimelightHelpers.setPipelineIndex(LimelightConstants.kLimelightName, pipeline);
        }
    ...
    /**
     * Determine which Limelight pipeline to use based on robot position relative to hub.
     * From the driver's perspective (birds eye view):
     * - Pipeline 1: Robot is on the LEFT side of the hub (>30 degrees CW from hub center)
     * - Pipeline 2: Robot is on the RIGHT side of the hub (>30 degrees CCW from hub center)
     * - Pipeline 0: Robot is centered (within ±30 degrees)
     */
    private int getPipeline() {
        ...
        // Calculate angle from hub to robot (in field coordinates)
        double dx = robotX - FieldConstants.kHubXMeters;
        double dy = robotY - FieldConstants.kHubYMeters;
        double angleToRobotDegrees = Math.toDegrees(Math.atan2(dy, dx));

        // For red alliance, the driver's perspective is flipped (looking toward -X)
        // so we negate the angle to maintain consistent left/right behavior
        boolean isRedAlliance = edu.wpi.first.wpilibj.DriverStation.getAlliance()
                .orElse(edu.wpi.first.wpilibj.DriverStation.Alliance.Blue)
                == edu.wpi.first.wpilibj.DriverStation.Alliance.Red;
        if (isRedAlliance) {
            angleToRobotDegrees = -angleToRobotDegrees;
        }

        if (angleToRobotDegrees > FieldConstants.kPipelineZoneThresholdDegrees) {
            // Robot is on the LEFT side of the hub (from driver perspective)
            return 1;
        } else if (angleToRobotDegrees < -FieldConstants.kPipelineZoneThresholdDegrees) {
            // Robot is on the RIGHT side of the hub (from driver perspective)
            return 2;
        } else {
            // Robot is centered relative to hub
            return 0;
        }
    }
}
